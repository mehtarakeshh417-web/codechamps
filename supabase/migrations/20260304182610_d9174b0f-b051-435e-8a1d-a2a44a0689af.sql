...
-- Fix ALL RLS policies across all tables to be PERMISSIVE instead of RESTRICTIVE
...
CREATE POLICY "Schools can view submissions" ON public.submissions FOR SELECT TO authenticated
  USING (assignment_id IN (
    SELECT id FROM assignments WHERE school_id IN (
...
-- Teachers need to read their school
...
-- Students need to read their school
...
-- Students can view their teacher
